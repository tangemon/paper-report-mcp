"""Search functionality for the arXiv paper."""
...
- Exclusions: "deep learning" ANDNOT ("survey" OR "review") to exclude survey or review papers
...
async def handle_search(arguments: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        for paper in client.results(search):
            logger.debug(f"get paper: {paper}")
        ...
        logger.error(f"ArXiv API error: {error_msg}")
        return [
            types.TextContent(type="text", text=f"ArXiv API Error:\n{error_msg}")
        ]
    except Exception as e:
        error_msg = get_exception_error()
        logger.error(f"Unexpected search error: {error_msg}")
